...
class AppleCookieDownloader(Processor):
    ...
    def download(self, url, curl_opts, output, request_headers, allow_failure=False):
      ...
      retcode = proc.poll()
      if retcode and not allow_failure: # Non-zero exit code from curl => problem with download
          curlerr = ''
          try:
    ...
    def main(self):
        ...
        login_cookies = pathname = os.path.join(download_dir, 'login_cookies')
        download_cookies = pathname = os.path.join(download_dir, 'download_cookies')
        ...
        login_curl_opts = [
          '--request', 'POST',
          '--data', '@{}'.format(self.env['login_data']),
        ...
        try:
          with open(output) as f:
            ...
            raise ProcessorError("Unable to list downloads. Check your Apple credentials.")
        except ValueError:
          pass
        ...
        proc = subprocess.Popen(gunzip_cmd, shell=False, bufsize=1,
                                stdin=subprocess.PIPE,
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE)
        ...
        if proc.returncode:
          gzerr = stderr.rstrip('\n')
          raise ProcessorError( "Gunzip failure: %s (exit code %s)" % (gzerr, proc.returncode) )
