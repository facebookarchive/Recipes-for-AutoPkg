...
class ChefTemplate(Processor):
  description = ("Produces a template Chef block. See "
                 "https://docs.chef.io/resource_template.html.")
  input_variables = {
    "resource_name": {
      "required": True,
      "description": ("Name for the resource. This can be a single "
    ...
    "force_unlink": {
      "required": False,
      "description": ("How the chef-client handles certain situations when "
    ...
    "helper": {
      "required": False,
      "description": "Define a helper method inline."
    },
    "ignore_failure": {
      "required": False,
      "description": ("Continue running a recipe if a resource fails for "
                      "any reason.")
    },
    "inherits": {
      "required": False,
      "description": ("Windows only. Whether a file inherits rights from "
                      "its parent template.")
    },
    "local": {
      "required": False,
    ...
    "manage_symlink_source": {
      "required": False,
      "description": ("Cause the chef-client to detect and manage the "
                      "source file for a symlink.")
    },
    "mode": {
      "required": False,
      "description": ("A quoted 3-5 character string that defines the "
                      "octal mode.")
    },
    "notifies": {
      "required": False,
      "description": ("Which resource takes action when this resource's "
                      "state changes.")
    },
    "owner": {
      "required": False,
    ...
    "retries": {
      "required": False,
      "description": ("The number of times to catch exceptions and retry "
                      "the resource.")
    },
    "retry_delay": {
      "required": False,
    ...
    "rights": {
      "required": False,
      "description": ("Microsoft Windows only. The permissions for users and "
                      "groups in a Microsoft Windows environment.")
    },
    "sensitive": {
      "required": False,
      "description": ("Ensure that sensitive resource data is not logged "
                      "by the chef-client.")
    },
    "source": {
      "required": True,
      "description": ("The name of the file in COOKBOOK_NAME/files/default "
                      "or the path to a file located in COOKBOOK_NAME/files.")
    },
    "subscribes": {
      "required": False,
      "description": ("Specify that this resource is to listen to another "
                      "resource, and then take action when that resource's "
                      "state changes..")
    },
    "variables": {
      "required": True,
      "description": ("A Hash of variables that are passed into a Ruby "
                      "template file.")
    },
    "verify": {
      "required": False,
      "description": ("A block or a string that returns true or false. A "
