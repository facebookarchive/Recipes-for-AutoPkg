...
class ChefFile(Processor):
  ...
  input_variables = {
    ...
    "file_sensitive": {
      "required": False,
      "description": ("Ensure that sensitive resource data is not logged by "
                      "the chef-client.")
    },
    "file_subscribes": {
      "required": False,
  ...
  def main(self):
    ...
    if len(self.env['resource_name'].split(',')) == 1:
      if self.env.get('%sresource_array' % prefix):
        ...
        self.env['chef_block'] = indent_block + block_name + ' ' + \
          self.env['resource_name'] + each_do_end
      else:
        self.env['chef_block'] = indent_block + block_name + ' ' + \
          self.env['resource_name'] + ' do\n'
    else:
