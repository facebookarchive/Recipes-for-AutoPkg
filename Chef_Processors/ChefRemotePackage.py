...
class ChefRemotePackage(Processor):
  ...
  input_variables = {
    ...
    "app": {
      "required": False,
      "description": "Name of the app being installed."
    },
    "checksum": {
      "required": True,
      "description": "SHA256 checksum for package."
    },
    "cleanup": {
      "required": False,
      "description": ("Specify whether we should keep the "
                      "downloaded package.")
    },
    "pkg_name": {
      "required": False,
      "description": ("Name of the package if it is not the same as ",
                      "`app`-`version`, or if the name has spaces.")
    },
    "receipt": {
      "required": True,
      "description": ("The package receipt to determine if it's "
                      "already installed.")
    },
    "remote": {
      "required": False,
      "description": ("Specify whether we should try to "
                      "download the package.")
    },
    "version": {
      "required": True,
      "description": ("The version of the package receipt to determine "
