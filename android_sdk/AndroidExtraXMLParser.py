...
def find_value_in_dict(thedict, key):
  '''Iterate through a dict to find all matching keys, even if inside
      another dict'''
  resultlist = list()
...
class AndroidExtraXMLParser(Processor):
  ...
  input_variables = {
    ...
    "name": {
      "required": True,
      "description": "Item name to match."
    },
    "tags": {
      "required": True,
  ...
  output_variables = {
    "found_value": {
      "description": "Result of found attribute."
    }
  }
      ...
      for key, outputVar in self.env['tags'].iteritems():
        ...
        if key == 'uses-license':
          self.env[self.env['tags']['uses-license']] = \
            record.find('%s%s' % (schema, 'uses-license')).attrib['ref']
          self.output("Found license-ref: %s" %
                      self.env[self.env['tags']['uses-license']])
          continue
        # Look for revision - third special case
        if key == 'revision':
          major = \
            find_value_in_dict(result_dict, 'revision')[0][0].get('major', '')
          minor = \
            find_value_in_dict(result_dict, 'revision')[0][1].get('minor', '')
          micro = \
            find_value_in_dict(result_dict, 'revision')[0][2]['micro']
          self.env[self.env['tags']['revision']] = \
